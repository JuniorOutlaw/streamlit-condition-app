...
# --- Sensor Dropdown ---
sensor_options = {
    "MX201": "forecast_status_output.xlsx",
    "DT203": "forecast_status_output_DT203.xlsx"
}
selected_sensor = st.selectbox("🧭 Select Sensor", options=list(sensor_options.keys()), index=0)
st.markdown(f"**📡 Viewing data for sensor:** `{selected_sensor}`")

# --- Load Data Based on Sensor ---
filename = sensor_options[selected_sensor]
try:
...
# --- Convert ForecastStart to datetime ---
df['ForecastStart'] = pd.to_datetime(df['ForecastStart'], errors='coerce')
df = df.dropna(subset=['ForecastStart'])  # Drop invalid timestamps
df = df.sort_values("ForecastStart").reset_index(drop=True)

# --- Forecast Timeline and Anomalous Points ---
forecast_times = df['ForecastStart'].dt.to_pydatetime().tolist()
anomalous_indices = df[df['Status'] == 'Anomalous'].index.tolist()

# --- Init Session State ---
if 'selected_time' not in st.session_state or st.session_state.get('last_sensor') != selected_sensor:
    st.session_state.selected_time = forecast_times[-1]
    st.session_state.last_sensor = selected_sensor

# --- Layout: Prev/Next Anomalous Buttons ---
col1, col2 = st.columns(2)
...
with col1:
    if st.button("⬅️ Previous Anomalous Point"):
        current_index = df[df['ForecastStart'] == st.session_state.selected_time].index[0]
        prev = df[(df.index < current_index) & (df['Status'] == "Anomalous")]
        if not prev.empty:
            st.session_state.selected_time = prev.iloc[-1]['ForecastStart']
            st.rerun()
        else:
            st.warning("🚫 No previous anomalous points found.")
...
with col2:
    if st.button("➡️ Next Anomalous Point"):
        current_index = df[df['ForecastStart'] == st.session_state.selected_time].index[0]
        next = df[(df.index > current_index) & (df['Status'] == "Anomalous")]
        if not next.empty:
            st.session_state.selected_time = next.iloc[0]['ForecastStart']
            st.rerun()
        else:
            st.warning("🚫 No more anomalous points found.")
...
try:
    ...
    st.error("⚠️ Failed to render timeline slider.")
    st.stop()

# --- Get Selected Row ---
row = df[df['ForecastStart'] == selected_time]
if row.empty:
    row = df.iloc[[df['ForecastStart'].sub(selected_time).abs().argmin()]]
else:
    row = row.iloc[0]

status = row['Status']
time_until_fault = row['TimeUntilFault']
forecast_start = row['ForecastStart']
...
.healthy { background-color: #d1ffd6; }
.unhealthy { background-color: #ffe0e0; }
.time-box {
    padding: 1rem; border-radius: 10px; background-color: #f0f0f0;
    text-align: center; font-size: 22px; margin-top: 1rem;
}
</style>
""", unsafe_allow_html=True)

# --- Display Forecast Time (formatted) ---
def format_datetime(dt):
    day = dt.day
    suffix = "th" if 11 <= day <= 13 else {1:"st", 2:"nd", 3:"rd"}.get(day % 10, "th")
    return dt.strftime(f"%-d{suffix} %B %Y, %H:%M")

formatted_time = format_datetime(forecast_start)
st.markdown(f"""
<div style="text-align:center; font-size:26px; margin-top:20px;">
    🕒 <strong>Forecast Time:</strong> {formatted_time}
</div>
